  ...
  <table class="table stripped">
    ...
    <tbody>
      <tr v-for="row in filteredOrderedRows" @click="select($event, row)" :class="getRowClasses(row)">
        ...
        <td v-for="col in schema.columns" :class="getCellClasses(row, col)"> <span v-html="getCellValue(row, col)"></span><span class="labels" v-if="col.labels != null">
            <div class="label" v-for="label in col.labels(row)" :class="'label-' + label.type">{{ label.caption }}</div></span></td>
      </tr>
    </tbody>
    <tfoot>
      <tr>
